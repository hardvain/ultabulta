...
}), angular.module("ngSpinkitImagePreloader", []).directive("spinkitImagePreloader", ["$compile", "$injector", function (a, b) {
  return {
    restrict: "A",
    scope: {ngSrc: "@", spinkitImagePreloader: "@", spinkitImagePreloaderClass: "@"},
    link: function (c, d, e) {
      var f, g, h = c.spinkitImagePreloaderClass || "spinner-wrapper";
      b.has(e.$normalize(c.spinkitImagePreloader) + "Directive") && (f = angular.element("<div/>").addClass(h), g = a("<" + c.spinkitImagePreloader + "/>")(c), f.append(g), f.css("overflow", "hidden"), d.after(f), d.css("width") && f.css("width", d.css("width")), e.width && f.css("width", e.width + "px"), d.css("height") && f.css("height", d.css("height")), e.height && f.css("height", e.height + "px"), d.on("load", function () {
        f.css("display", "none"), d.css("display", "block")
      }), c.$watch("ngSrc", function () {
        f.css("display", "block"), d.css("display", "none")
      }))
    }
  }
}]), angular.module("angular-spinkit").run(["$templateCache", function (a) {
